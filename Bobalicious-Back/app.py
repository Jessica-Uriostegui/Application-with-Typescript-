...
def update_product(product_id):
    ...
    try:
        product_data = request.form.to_dict()  # Get form data
        if filename:
            product_data['image_url'] = filename  # Update the image path if a new image was uploaded
        product_data = product_schema.load(product_data, partial=True)  # Validate and load data
    except ValidationError as err:
        return jsonify(err.messages), 400
    ...
    try:
        with Session(db.engine) as session:
            ...
            if 'products' in order_data:
                ...
                products_query = select(Product).filter(Product.product_id.in_(product_ids))
                products = session.execute(products_query).scalars().all()
